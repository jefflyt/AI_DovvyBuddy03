...
DovvyBuddy is a web-based AI assistant that helps **recreational divers** research, plan, and prepare for dive trips. It provides:
...
- **Qualified lead capture** for partner dive shops/schools (context-rich handoff), but **no booking flow**.

The initial public version is a **web chatbot only**, with intentionally small, high-quality coverage.

### 1.2 Goals

- G-01: Help recreational divers quickly narrow down **where** and **when** to dive, based on their profile and constraints.
- G-02: Provide **trustworthy, destination-specific information** about dive sites, conditions, highlights, logistics, and high-level safety.
- G-03: Generate **qualified, context-rich leads** for partner dive shops/schools.
- G-04: Validate demand with a **limited set of destinations** before scaling.
- G-05: Provide confidence-building support for **potential new divers** considering PADI Open Water (or equivalent) certification.
- G-06: Generate leads for **dive training programs**, not just trips.
...
- RAG over curated content for:
  ...
  - **5–10 dive sites per destination**.
...
- **Inline lead capture** (minimal fields) and automated delivery to partner(s).
- Basic analytics (sessions, key funnels, leads).
...
### P-01 – Holiday Open Water Diver

- **Profile:** Open Water certified, ~10–40 logged dives.
- **Goals:** Easy–moderate holiday diving; seasonality/conditions clarity; reassurance.
- **Success:** Confident destination choice + warm handoff to a reputable operator.

### P-02 – Enthusiast Advanced/Rescue Diver

- **Profile:** ~50–200 dives; advanced interests.
- **Goals:** Nuanced conditions + sites matching interests (macro/wreck/sharks/drift).
- **Success:** High-context shortlist + matching shops.

### P-03 – Dive Shop Owner / Manager (Indirect Persona)

- **Goal:** Fewer, better-qualified leads with context.
- **Success:** Lead includes enough info to quote/advise without repeated back-and-forth.

### P-04 – Prospective New Diver

- **Goal:** Understand what learning involves; reduce fear; find a reputable school.
- **Success:** Clarity + optional training lead submitted.

---

## 3. Key Use Cases (Lean)

### UC-01 — Discover Suitable Destination (Covered Set Only)
...
### UC-02 — Research a Covered Destination / Dive Sites

- RAG-grounded destination overview, conditions, highlights.
- Suggest 2–5 relevant sites (from the covered set).

### UC-03 — Simple Trip Outline (Optional in V1; Required in V1.1)
...
### UC-04 — Capture and Send a Qualified Lead

- Offer 1–3 partner shops.
- Collect minimal lead details, then send via email/webhook.

### UC-05 — Safety / No-Fly / Medical Queries
...
### UC-06 — Learn to Dive (V1: Minimal)

- Explain the PADI Open Water (or equivalent) PADI Open Water (or equivalent) course structure and common fears.
- Offer training lead capture (minimal fields).
...
- ≥ 2 qualified leads/week by end of month 2.
- Qualitative: users report the answers feel grounded and limitations are clear.
- At least 1 partner shop confirms the lead quality is higher than generic inquiries.

---

## 11. Acceptance Criteria (Adjusted)

- Covered-set recommendations only.
- Out-of-scope responses clearly decline and redirect.
- Lead submission works end-to-end, and failures are visible to the founder.
